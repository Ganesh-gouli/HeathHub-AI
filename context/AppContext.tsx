...
import { registerUser, logUserActivity, logWorkout, fetchUserActivity } from '../services/userService';
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const login = async (profile: Omit<UserProfile, 'bmi'>) => {
        ...
        if (newUser.email) {
            ...
            try {
                const data = await fetchUserActivity(newUser.email);
                if (data && data.history) {
                    setLogHistory(data.history);
                } else {
                    // If we get here, data might be null due to fetch error caught in service but returned as null
                    if (data === null) throw new Error("Could not connect to server");
                }
            } catch (err) {
                console.error("Context fetch error:", err);
                setHistoryError("Failed to load activity history. Ensure server is running.");
            } finally {
    ...
    const removeFoodItem = useCallback((itemToRemove: LoggedFood) => {
        ...
        if (indexToRemove > -1) {
            ...
            updateLogHistory(updatedLog);
            // Note: Removing from backend is not yet supported in this simple implementation
        }
    }, [todayLog]);
